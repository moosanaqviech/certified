...
2. The **payload** block between the `LESSON PAYLOAD — BEGIN / END` markers (`A` and `cards`).
...
- `last` (optional): `true` on the final card. Turns the button into Restart and resets all quizzes.
